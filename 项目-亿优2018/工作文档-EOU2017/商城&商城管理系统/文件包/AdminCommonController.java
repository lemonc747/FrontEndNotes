...
import org.weiyun.common.util.RequestMapUtils;
...
public class AdminCommonController extends BaseController {
	...
	protected Map getParameterMap() throws Exception {
		try {
			...
			map = RequestMapUtils.toMap(map);
			Map sessionUser = (Map) getSession().getAttribute("session_user");
			if (sessionUser == null) {
				this.getResponse().sendRedirect("/admin/login.jsp");
			}
			map.put("managerId", sessionUser.get("managerId"));
			return map;
	...
	@Override
	protected Map getUserInfo() throws Exception {
		if (getSession().getAttribute("session_user") == null) {
			this.getResponse().sendRedirect("/admin/login.jsp");
			throw new Exception();
		}
		Map userInfo = (Map) getSession().getAttribute("session_user");
		return userInfo;
	}
