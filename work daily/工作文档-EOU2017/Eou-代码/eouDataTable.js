...
;(function($){
    ...
    var ETable = function(elements, options){
        ...
        if( options && !this._CheckAndInitStorage(options) ){
    ...
    ETable.prototype = {
        init: function(params){
            createDatatable( params || this.$el.data(KEY_TABLE_ITEMS) );
            return this;
        },
        ...
        _CheckAndInitStorage: function(params){
            // 检查table参数是否完整有效
    ...
    function getLSDataChild(tableId, keyWord){
        ...
        if(!localStrg || typeof localStrg !== "object" ){ return false }
    ...
    function initLocalStorage(tableId,lsData){
        if(!lsData || typeof lsData !== "object" ){ return false }
    ...
    function initTabContainerHtml(contentId, toolsAreaId, custom, permissions,imports, exports){
        ...
        if(custom && isArray(custom)){
            for( var i=0;i<custom.length; i++){
                var obj = custom[i], name = obj.name, location = obj.location, color = obj.color, icon = obj.icon, associated = obj.associated;
    ...
    function queryTableDataModel(tableId, page){
        ...
        var params = {
            url: url,
            data: searchParams,
            success: function(req){var newResult = req.data,
    ...
    function priceFormatHighLight(value){
        ...
        switch(decimals){
            ...
            default: value = Math.ceil(value*100)/100;
        }
    ...
    function trafficCommonFM(value){
        ...
        switch(decimals){
            ...
            default: value = Math.ceil(value*1000)/1000;
        }
        return value;
    }
