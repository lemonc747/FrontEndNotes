...
;(function ($) {
    ...
    var KEY_LS_ETABLE = "localStorage.etable.",
        // KEY_LS_COLS_SHOW = "showCols",
        // KEY_LS_VER = "version",
        KEY_LS_COLS_RESET = "cols_reset",
        // KEY_JQDATA_OPTIONS = "keyJQDataOptions",
        KEY_FLAG_CLASS_ETABLE = "class_flag_destroy_table",
        KEY_FLAG_CLASS_ETABLE_COMPONENTS = "class_flag_destroy_table_components";

    var ETable = function (elements, options) {
        $(this).addClass(KEY_FLAG_CLASS_ETABLE);
        this.tableId = elements.id;
        this.$el = $(elements);
        this.options = options;
        // this.resetOptions = options;

        if (!this._CheckAndInitOptions()) {
            layer.msg.error("Table params is error! tableID is " + (options && options.tableId) || "missing!");
            return {};
        }
    };
    ...
    ETable.prototype = {
        ...
        createTable: function () {
            createDatatable( this.options );
            return this;
        },
        // //不再兼容旧版本
        // //兼容旧版参数，这里比较特殊，在$.etable()时不需要存储参数，而是在绘制表格时来做
        // initCompatibleVersion: function (tableId, toolsAreaId, urlPrefix, params, permissions, page, customSearchWidget) {
        //     if (params) {
        //         params.initParams = {};
        ...
        //     }
        //     this._CheckAndInitStorage(params);
        ...
        //_CheckAndInitStorage判断参数合法性，若合法执行存储，若不合法返回false
        _CheckAndInitOptions: function () {
            // check options
            var options = this.options,
                initParams = options && options.initParams ? options.initParams : {},
                tableId = initParams.tableId,
                toolsId = initParams.toolsId,
                urlPrefix = initParams.urlPrefix,
                permissions = initParams.permissions;
            if (!options || !tableId || !toolsId || !urlPrefix || !permissions) {
                return false;
            }
            //init options and localStorage
            var keyLS = KEY_LS_ETABLE + this.tableId,
                localStorage = this._formatOptions(),
                oldStorage = getLS(keyLS);
            if (!(oldStorage && (!(oldStorage.version || localStorage.version)
                || (oldStorage.version && localStorage.version && oldStorage.version === localStorage.version)))) {
                setLS(keyLS, localStorage);
            }
            return true;
        },

        _formatOptions: function(){
            var options = formatTableParams(this.options);
            /* 此时的options：
             * 更新第一层参数(eg:urlPrefix) & 更新trs & 新增了localStorage
             * localStorage: 作为副本？使用时直接从这里取，但是更新时需要同步维护本地和远程
             */
            this.options = $.extend({}, this.options, options);
            this.options.tableId = this.tableId;
            return options.localStorage;
        }
    };

    $.fn.etable = function (options, params) {
        return this.each(function () {
            var $this = $(this),
                data = $this.data("etable");
            options = typeof options === "object" && options;
            if (!data) {
                data = new ETable(this, options);
                $this.data("etable", data);
            }
    ...
    //摧毁DOM
    $.fn.etable.destroyETable = function(jqSelector){
        $(jqSelector).find("."+KEY_FLAG_CLASS_ETABLE).remove();
        $(jqSelector).find("."+KEY_FLAG_CLASS_ETABLE_COMPONENTS).remove();
    };

    function formatTableParams(options) {
        var tableItems = $.extend({}, options);
        ...
        result.localStorage = localStrg;
        result.trs = newItems;
        result.urlPrefix = urlPrefix;
        return result;
    }

    function setLS(keyWord, data) {
        ...
        if (typeof data === "string") {
            storage.setItem(keyWord, data);
        } else if (typeof data === "object") {
            storage.setItem(keyWord, JSON.stringify(data));
        }
    }

    function getLS(KEY_WORD) {
        var data = window.localStorage.getItem(KEY_WORD);
        if (data !== null && data !== "") {
    ...
    function getLSDataChild(tableId, keyWord) {
        var localStrg = getLS(tableId, KEY_TABLE_LS);
    ...
    function setLSDataChild(tableId, keyWord, data) {
        var key = "config." + KEY_TABLE_LS + "." + tableId,
            ...
            localStrg = getLS(tableId, KEY_TABLE_LS);
    ...
    function checkLocalStorage(tableId) {
        var params = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
    ...
    function createDatatable(options, customSearchWidget) {
        var tableId = options.tableId,
            toolsId = options.toolsId,
            permissions = options.permissions,
            custom = options.customComps && options.customComps.custom;
        initTabContainerHtml(tableId, toolsId, custom, permissions, options.imports, options.exports);
        initTableTools(tableId, options, customSearchWidget);
        initTableBodyAndEvent(tableId, 1);
    ...
    function initTableTools(tableId, params, customSearchWidget) {
        var localStrg = getLS(tableId, "localStrg"),
    ...
    function initSearchWidgetModel(tableId) {
        var $searchForm = $("#" + tableId + "-searchArea"),
            tableParams = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
    ...
    function resetSearchCondition(tableId) {
        var columns = $("#" + tableId).data(KEY_JQDATA_OPTIONS).trs,
    ...
    function changeSelectRows(tableId) {
        var chsubList = $("input[name*='" + tableId + "-rowItems']").length,
            ...
            tableItems = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
    ...
    function rowsSort(tableId, columnName) {
        var tableParams = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
    ...
    function changePagesizeModal(tableId) {
        ...
        // setLS(tableId, "pageSize", pageSize);
        queryTableDataModel(tableId, 1);
    }
    ...
    function createTabEntityById(tableId) {
        var items = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
    ...
    function resetToolsBtnStatus(tableId) {
        var tableItems = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
            custom = tableItems.customComps && tableItems.customComps.custom;
        if (custom && isArray(custom)) {
            for (var i = 0; i < custom.length; i++) {
                var obj = custom[i], name = obj.name, color = obj.color, associated = obj.associated,
...
//新增 && 单行编辑 && 多行编辑
    ...
    function createBatchEditModal(tableId, rows) {
        var $table = $("#" + tableId),
            tableLocal = $table.data(KEY_JQDATA_OPTIONS),
...
//     }else{
//         var cols = $("#"+tableId).data(KEY_JQDATA_OPTIONS).trs;
...
// }
    ...
    function createEditDialog(tableId, type, number) {
        var tableLocal = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
            tableItems = tableLocal.trs,
            tableKey = tableLocal.tableKey,
            i18nPrefix = tableLocal.i18nPrefix,
...
//初始化modal内容和数据
    function fillModalContent(tableId, type, number) {
        var $table = $("#" + tableId),
            $modal = $("#" + tableId + "-" + type),
            local = $table.data(KEY_JQDATA_OPTIONS),
...
//初始化批量编辑modal内容和数据
    ...
    function toolDelete(tableId) {
        var urlPrefix = $("#" + tableId).data(KEY_JQDATA_OPTIONS).initParams.urlPrefix,
            params = {
    ...
    function getSubmitData(params) {
        var tableId = params.tableId,
            ...
            tableLocal = $("#" + tableId).data(KEY_JQDATA_OPTIONS),
            tableItems = tableLocal.trs,
            tableKey = tableLocal.tableKey,
    ...
    function initEditFormValidator(tableId, type, submitFunction) {
        var tableItems = $("#" + tableId).data(KEY_JQDATA_OPTIONS).trs,
    ...
    function thOnMouseUpModel(tableId) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (name == tableStartColumnName) {
                ...
                setLS(tableId, "tableParams", tableItems);
                createTabEntityById(tableId);
            }
        }
        var $body = $("body");
